...
import { getFieldErrors, profileSchema, type ProfileFormValues } from '@/lib/validation';
...
const errorInputClassName = 'border-danger focus:border-danger focus:ring-2 focus:ring-danger/20';
  ...
  return (
    <section className="min-h-[676px] md:relative">
      ...
      <div className="rounded-xl bg-card p-4 shadow-card md:h-[337px] md:w-[580px]">
        ...
        <form className="grid gap-4 md:gap-3" noValidate onSubmit={handleSubmit(submitForm)}>
          ...
          {formMessage ? <p className="text-sm leading-5 text-danger">{formMessage}</p> : null}
          {successMessage ? <p className="text-sm leading-5 text-success">{successMessage}</p> : null}

          <div className="mt-4 grid gap-4 md:mt-3 md:flex md:gap-2">
            <button className="h-9 rounded-md border border-primary bg-card px-4 text-sm leading-5 font-bold text-primary md:h-10 md:text-base md:leading-6" onClick={handleCancel} type="button">
              Отменить
            </button>
            <button
